...
def create_cart1():
    try:
        ...
        db.cart1.create_index([('name', pymongo.ASCENDING)],unique=True)
        dbResponse = db.cart1.insert_one(item)
...
@app.route("/cart2", methods=["POST"])
def create_cart2():
    try:
        item = {"name":request.form["name"],
                "price":request.form["price"],
                "quantity":request.form["quantity"]}
        db.cart2.create_index([('name', pymongo.ASCENDING)],unique=True)
        dbResponse = db.cart2.insert_one(item)
        print(dbResponse.inserted_id)
        return Response(
            response=json.dumps(
                {"message":"item added to cart",
...
#####################################
# Delete cart
@app.route("/cart2", methods=["DELETE"])
def delete_cart():
    try:
        dbResponse = db.cart2.delete_many({})
        if (dbResponse.deleted_count > 0):
            return Response(
                response=json.dumps(
...
def get_quantity_range(min_quan,max_quan):
    query = {"$and":[
            {"quan": {"$gte":min_quan}},
            {"quan": {"$lte":max_quan}}
        ]}
    cart = cart1_collection.find(query).sort("quan")
    for c in cart:
        printer.pprint(c)

get_quantity_range(20,35)